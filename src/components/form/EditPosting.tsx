...
const EditPosting = ({ post }: any) => {
  ...
  const handlePublish = async (data: any) => {
    ...
    try {
      ...
      postForm.append("creatorId", formdata.creator._id);
  ...
  return (
    <form
        ...
        <label
          ...
          {selectedFile ? (
            <div className="relative rounded-lg">
              ...
              />
              {imgUploading && (
                <div className="rounded-lg absolute bg-white opacity-70 top-0 left-0 w-full h-full text-black flex justify-center items-center">
      ...
      {!isUploading ? (
        imgUploading ? (
          <div className="py-3 rounded-lg mt-10 bg-[#ccc] opacity-50 text-light-1 w-full flex justify-center items-center bottom-24 ">
            Update
          </div>
        ) : (
          <button
